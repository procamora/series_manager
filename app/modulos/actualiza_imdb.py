...
class actualizaImdb():
    ...
    def __compruebaDatos(self, Datos, series):
        if str(series['imdb_Temporada']) == Datos['Temp'] and str(series['imdb_Finaliza']) == Datos['year'] and str(
                series['imdb_Capitulos']) == Datos['Cap']:
            return True
        else:
            return False
    ...
    def __actualizaDatos(self, Datos):
        query = 'UPDATE series SET imdb_Temporada="{}", imdb_Finaliza="{}", imdb_Capitulos="{}" WHERE imdb_id Like "{}"'.format(
            Datos['Temp'], Datos['year'], Datos['Cap'], Datos['id_imdb'])
    ...
    def __actualizaDatosParcial(self, Datos):
        query = 'UPDATE series SET imdb_Temporada="{}", imdb_Finaliza="{}" WHERE imdb_id Like "{}"'.format(
            Datos['Temp'], Datos['year'], Datos['id_imdb'])
        conectionSQLite(self.nombre_db, query)
        logger.info(query)
