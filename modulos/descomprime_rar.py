...
def descomprime(fichero):
    ...
    # partition(fichero.rar) = (antecedente, fichero.rar, consecuente), lo que hago es sacar el directorio del fichero
    ...
    if rar.needs_password():
        ...
        for i in os.listdir('{}'.format(dir)): # con glob no me lista los ficheros
...
def main(ruta):
    ...
    for i in glob.glob(ruta):
        print(i)
        try:
            a.sendTg('Empieza proceos descomprimir: {}'.format(i.split('/')[-1]))
            descomprime(i)
        except:
